...
def fetch_option_chain(symbol, expiry):
    ...
    for i, strike in enumerate(sel_strikes):
        ...
        ce_ltp = ce_ws.get("ltp") or ce_q.get("ltp", 0)
        pe_ltp = pe_ws.get("ltp") or pe_q.get("ltp", 0)
...
.spot-val{font-family:var(--mono);font-size:15px;font-weight:700;line-height:1.1;}
.spot-chg{font-family:var(--mono);font-size:10px;margin-top:3px;letter-spacing:.3px;}
.spot-chg.up{color:var(--up);}
.spot-chg.dn{color:var(--down);}
.spot-chg.fl{color:var(--muted);}
/* Spot wrapper layout */
#spotWrap{display:none;flex-direction:column;align-items:flex-start;justify-content:center;}
...
<div class="topbar">
  ...
  <!-- Spot display — left side, after symbol tabs -->
  <div id="spotWrap" style="display:none;padding:0 18px;border-left:1px solid var(--border2);">
    <div class="spot-val" id="spotVal">—</div>
    <div class="spot-chg fl" id="spotChg"></div>
  </div>
...
async function loadChain() {
  ...
  try {
    ...
    $('spotWrap').style.display = 'flex';
    $('spotVal').textContent=fmt(S.spot);
    // prevClose for index comes from first WS tick (tick.close = prev day close)
    // Reset spotChg until first tick arrives
    $('spotChg').textContent=''; $('spotChg').className='spot-chg fl';
  } catch(e){
...
function renderStats(data) {
  ...
  $('statsBar').innerHTML=`
    <div class="stat"><div class="stat-l">Spot</div><div class="stat-v">${fmt(S.spot)}</div></div>
...
if __name__ == "__main__":
    ...
    print()
    print("  Install deps:")
    print("  py -3.13 -m pip install flask flask-cors flask-socketio")
    print("  py -3.13 -m pip install websocket-client")
    print()
